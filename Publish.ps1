...
# Write-Host cannot be redirected or captured, so PSScriptAnalyzer rejects it. Write-Information
# carries the same text on the information stream, and $PSStyle supplies the colour that made
# Write-Host attractive for the release messages. $PSStyle needs PowerShell 7.2+; on older hosts
# the messages come through uncoloured rather than not at all.
function Write-Status {
	param(
		...
		[ValidateSet('Red', 'Yellow', 'Cyan', 'Green')]
		[string]$Colour
	)

	$text = $Message
	if ($Colour -and $PSStyle) {
		$text = "$($PSStyle.Foreground.$Colour)$Message$($PSStyle.Reset)"
	}
